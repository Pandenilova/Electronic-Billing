...
def save_products(products):
    if not products:
        ...
        df.to_csv(PRODUCT_FILE, index=False, encoding='utf-8') #unicode encoding ensures compatibility across different systems
        return
...
def show_bill(products, bill):
    ...
    for i, pid in enumerate(bill):
        ...
        total += product['price']
    print(f"Total: {total:.2f} DKK") #2 decimal places for currency displaySS
        ...
        if choice == '1':
            if not products:
               ...
               # for pid, p in products.items():
                ...
                print(f"{'ID':<8} {'Name':<15} {'Price in DKK':<20} {'Quantity':<10}")
                print("-" * 50)
                for pid, p  in products.items():
                    print(f"{pid}  {p['name']:<20} {p['price']:<20.2f} {p['quantity']:<10}")    
